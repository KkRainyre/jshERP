...
public class FunctionController extends BaseController {
    ...
    public JSONArray getMenuByFunction(List<Function> dataList, String fc, String approvalFlag,
            Map<Long, Long> funIdMap, User userInfo) throws Exception {
        JSONArray dataArray = new JSONArray();
        for (Function function : dataList) {
            // 如果不是超管也不是租户就需要校验，防止分配下级用户的功能权限，大于租户的权限
            if ("admin".equals(userInfo.getLoginName()) || userInfo.getId().equals(userInfo.getTenantId())
    ...
    public JSONArray getFunctionList(List<Function> dataList, String type, String keyId, List<Long> funIdList)
            throws Exception {
        JSONArray dataArray = new JSONArray();
        // 获取权限信息
